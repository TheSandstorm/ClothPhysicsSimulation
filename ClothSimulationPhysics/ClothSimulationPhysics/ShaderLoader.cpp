...
GLuint ShaderLoader::CreateProgram(const char* vertexShaderFilename, const char* fragmentShaderFilename)
{
	...
	else
	{
		...
		if (link_result == GL_FALSE)
		{
			...
			PrintErrorDetails(false, program, programFilename.c_str());
			return 0;
		}
...
{
	std::string programFilename = static_cast<std::string>(vertexShaderFilename) + static_cast<std::string>(geometryShaderFilename) + static_cast<std::string>(fragmentShaderFilename);
	std::map<std::string, GLuint>::iterator it = shaderMap.find(programFilename);
	if (it != shaderMap.end())
	{
		return (*it).second; // pair.second
	...
	else
	{
		...
		if (link_result == GL_FALSE)
		{
			//std::string programFilename = vertexShaderFilename + *geometryShaderFilename + *fragmentShaderFilename;
			PrintErrorDetails(false, program, programFilename.c_str());
			return 0;
		}
		programMap.insert({ programFilename, program });
...
{
	std::string programFilename = static_cast<std::string>(vertexShaderFilename) + static_cast<std::string>(TessControlShaderFilename) + static_cast<std::string>(TessEvalShaderFilename) + static_cast<std::string>(fragmentShaderFilename);
	std::map<std::string, GLuint>::iterator it = shaderMap.find(programFilename);
	if (it != shaderMap.end())
	{
		return (*it).second; // pair.second
	...
	{
		GLuint vertexShaderID = CreateShader(GL_VERTEX_SHADER, vertexShaderFilename);
		GLuint tessControlShaderID = CreateShader(GL_TESS_CONTROL_SHADER, TessControlShaderFilename);
		GLuint tessEvalShaderID = CreateShader(GL_TESS_EVALUATION_SHADER, TessEvalShaderFilename);
		GLuint fragmentShaderID = CreateShader(GL_FRAGMENT_SHADER, fragmentShaderFilename);
		GLuint program = glCreateProgram();
		glAttachShader(program, vertexShaderID);
		glAttachShader(program, tessControlShaderID);
		glAttachShader(program, tessEvalShaderID);
		glAttachShader(program, fragmentShaderID);
		glLinkProgram(program);
		...
		if (link_result == GL_FALSE)
		{
			//std::string programFilename = vertexShaderFilename + *TessControlShaderFilename + *TessEvalShaderFilename + *fragmentShaderFilename;
			PrintErrorDetails(false, program, programFilename.c_str());
			return 0;
		}
		programMap.insert({ programFilename, program });
